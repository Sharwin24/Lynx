...
class Ascii_Vizualizer:
    ...
    def populate(self):
        for i in range(len(self.path)):
            step_string = self.maze_string[:self.path[i]
                                           ] + "*" + self.maze_string[self.path[i]+1:]
...
class Visualizer:
    ...
    def display_maze(self):
        ...
        top_left = pygame.Vector2(
            screen.get_width() / (cols*2), screen.get_height() / (rows*2))
                ...
                if mlist[j*cols + i].is_wall:
                    ...
                    if (j*cols + i) == start:
                        ...
                        start_text_rect.center = (
                            top_left.x + i*screen.get_width()/cols, top_left.y + j*screen.get_height()/rows)
                    elif (j*cols + i) == goal:
                        color = "red"
                        goal_text = font.render('Goal', True, "black")
                        goal_text_rect = goal_text.get_rect()
                        goal_text_rect.center = (
                            top_left.x + i*screen.get_width()/cols, top_left.y + j*screen.get_height()/rows)
                    else:
                        color = "white"
            ...
            if not filled_in:
                if not (self.path[path_index] == goal or self.path[path_index] == start):
                    pygame.draw.rect(screen, "cyan", pygame.Rect((top_left.x + (self.path[path_index] % cols)*screen.get_width()/cols) - rect_width/2,
                                                                 (top_left.y + (self.path[path_index]//cols)*screen.get_height(
                                                                 )/rows) - rect_height/2,
